...
function SubmitButton({ className = '', text = 'submit', size = 'lg' }: SubmitButtonProps) {
    ...
    return (
        <Button type='submit' disabled={pending} className={cn('capitalize', className)} size={size}>{pending ? <> <ReloadIcon className='mr-2 w-4 h-4 animate-spin' /> Please wait... </> : text}</Button>
    )
}
    ...
    const renderIcon = () => {
        switch (actionType) {
            ...
            default:
                ...
                throw new Error(`Invlaid action type:${never}`)
        }
    }
...
export const CardSubmitButton = ({ isFavorite }: { isFavorite: boolean }) => {
    ...
    return (
        <Button type='button' size='icon' variant='outline' className='p-2 cursor-pointer'>
